...
**Good:** "3 files, apply in order: (1) `HtmlReportWriter.java` — emit `{{scanStats}}` token; (2) `report-template.html` — reference the token in the header meta-grid; (3) `report-styles.css` — style the new meta row. All diffs below."

### Rollback message

**Bad:** *(silently reverts)*

**Good:** "Reverting `HtmlReportWriter.java` and `report-template.html` to the last approved state (commit `<sha>`). Ready for next instruction."
...
| `HeaderBar`, `KpiCardPanel`, `SeverityTabs`, `FooterBar`, `FindingsTableModel`, `FindingsTableRenderer`, `FindingsContextMenu` | ui.dialog     | Sub-components. `SeverityTabs.setFilterListener(Consumer)` and `KpiCardPanel.setSelectionListener(Consumer<Set<Category>>)` both drive `FindingsTableModel`, which composes severity × category filters in `rebuildView`; `countAll`/`countSeverity` also respect the category filter. `KpiCardPanel` is a compact row of native-L&F `JToggleButton`s — label `"<Category> (N)"` with a 10×10 `ColorSquareIcon` in the category color, no custom border (pressed/raised state comes from the L&F so toggle state is visually unambiguous). Tab-traversable, Space/Enter toggles. `toggleCategory(Category)` is the programmatic hook driven by `Alt+1..6`. `SeverityTabs` labels are `All / High / Medium / Low` (UI display only — `Severity` enum + JSON stays `error / warn / info` per invariant 1). `HeaderBar` has no title label — the window title is set by `AuditDialog.applyWindowTitle` to `JAuditor-<jmx> [•]`. `HeaderBar` exports via a single split-button opening a `JPopupMenu` (HTML/JSON). `FindingsTableRenderer` sets `toolTipText` to the breadcrumb so long paths are readable. All explicit fonts derive from `UIManager.getFont("Label.font")` (no hard-coded pt, no `MONOSPACED`) so JMeter zoom/scale propagates. |
...
| `HtmlReportWriter`, `HtmlTemplate`, `HtmlEscaper`                                                                              | export.html   | Token substitution on `report-template.html`; `/*STYLES*/` → `report-styles.css`, `/*XLSX*/` → inlined `xlsx-style.bundle.js`. Escaper handles `< > & " '`. Writer emits Summary (category cards + `<details>`-wrapped rule reference) + one panel per category with findings. No "All findings" master tab; no footer; header carries only Test Plan + Scan Analysis Date plus a right-aligned toolbar (Export Excel, Dark Mode). Sort key "severity" uses `data-sort="1|2|3"` (ERROR<WARN<INFO). Findings grouped by rule id when `≥ HtmlReportWriter.GROUP_THRESHOLD` share an id — head row marked `grp-head`, rest `grp-member` with `hidden`. Uses `data-category="<slug>"` attributes over inline hex; CSS owns light, `prefers-color-scheme: dark`, and explicit `html[data-theme="dark\|light"]` palettes. `HtmlReportWriter.write(ScanResult, String pluginVersion, Path)` keeps the `pluginVersion` parameter for API stability; unused by the HTML path, still consumed by `JsonReportWriter`. `ThemeColors.cssHexLight` / `cssHexDark` are unused from the writer as of this refactor — palettes live in CSS — but the helpers stay as public API.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
- **HTML** (`report-template.html` + `report-styles.css` + bundled `xlsx-style.bundle.js`, classpath). Single
  ...
  groups. Sentinel tokens (non-`{{}}`): `/*STYLES*/`, `/*XLSX*/`. Writer tokens: `{{title}}`, `{{jmxFileName}}`,
...
| `io/.../jauditor/report/xlsx-style.bundle.js`                | xlsx-js-style bundle (inlined via `/*XLSX*/`)    |
