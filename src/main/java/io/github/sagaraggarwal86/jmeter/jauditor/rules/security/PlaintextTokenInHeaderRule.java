    ...
    public List<Finding> check(JMeterTreeNode node, ScanContext ctx) {
        ...
        for (int i = 0; i < hm.getHeaders().size(); i++) {
            ...
            out.add(make(ctx.pathFor(node),
                    ...
                    "This Header Manager sends an Authorization header with a bearer token written directly into the .jmx file (masked here as " + io.github.sagaraggarwal86.jmeter.jauditor.util.JAuditorLog.redact(val) + " — JAuditor never prints token contents). Anyone who opens the test plan — teammates, reviewers, anyone with access to the source repository — can read the real token. Tokens committed into test plans have a habit of staying valid long after the author meant to rotate them, and they often end up leaking into screenshots, CI logs, or chat messages.",
