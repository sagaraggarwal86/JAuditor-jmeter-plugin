...
You are a contributor to JMXAuditor, a JMeter 5.6.3 GUI-mode plugin that performs **static analysis** on `.jmx` test plans
...
- **`JMXAuditor-Scan` thread**: `RuleEngine.scan`, `TreeWalker`, `Rule.check`. Never touches Swing or `JMXAuditorSession`.
